...
yao %>%
  ...
  summarise(total_days_missed = sum(n_days_miss_work, na.rm = TRUE))  # Add na.rm = TRUE to get sum
```
...
yao %>%
  ...
  group_by(sex, neighborhood) %>%  # Select multiple variables in group_by() function
...
yao %>% 
  ...
  select(mean_age) # doesn't work as expected because the sex column is grouped with mean_age column
```
...
yao %>%
  ...
  count(sex, occupation)  # A compact way to count data by nested groups
```
...
yao_no_male_children %>% 
  ...
  summarise(number_of_individuals = n())  # Does not produce an output for male children (which are not present)
```
...
yao_no_male_children %>% 
  ...
  summarise(count = n())  # Now produces an output for male children
```
