...
```{r}
yao_sex_weight %>% arrange(weight_kg)
```
...
yao_sex_weight %>% 
  group_by(sex) %>% 
  arrange(weight_kg, .by_group = TRUE)  # Now data is arranged with all females first, then males
```
...
yao_sex_weight %>% 
  group_by(sex) %>% 
  filter(weight_kg == max(weight_kg))  # Returns the max weight for each group in the data set
```
